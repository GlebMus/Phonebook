...
from models import add_data_to_contacts, contacts, edit_contact, find_contact


def start_point() -> None:
    """
    Функция start_point - стартовая точка программы,
    срабатывает при запуске скрипта и обрабатывает дальнейшие
    действие пользователя :return: None
    """

    operations = {
        1: _show_all_contacts_operation,
        2: _add_new_contact_operation,
        3: _edit_contact_operation,
        4: _find_contact_by_parameters_by_parameters
    }
    ...
    action = operations.get(first_operation)
...
def _show_contacts(generator: Iterator) -> None:
    ...
    и при вызове выводит в консоль (если не указаны иной stdout)
    определенное кол-во контактов (Указаны в генераторе)
    :param generator:
    :return: None
    """
...
def _show_all_contacts_operation() -> None:
    """
    Функция _show_all_contacts_operation
    выполняет операцию 1 (Выводит постранично записи из справочника)
    :return: None
    """
    while True:
        try:
            ...
            break
        client_answer = input(
            "Для того, чтобы вывести следущую страницу "
            "нажмите enter, для того чтобы выйти нажмите q\n")
        if client_answer == 'q':
            break
...
def _add_new_contact_operation() -> None:
    """
    Функция _add_new_contact_operation
    выполняет операцию 2 (Добавление
    новой записи в справочник)
    :return: None
    """
    last_name, first_name, surname = input("Введите фамилию имя и отчество "
    ...
    add_data_to_contacts(first_name, last_name, surname,
                         organization, personal_phone, home_phone)


def _edit_contact_operation() -> None:
    """
    Функция _edit_contact_operation
    выполняет операцию 3 (Возможность редактирования записей в справочнике)
    :return: dict
    """
    contact = _find_contact_by_parameters()
    (last_name,
     first_name,
     surname) = input("Введите новые фамилию имя "
                      "и отчество изменяемого контакта:").split()
    ...
    edit_contact(contact, first_name, last_name, surname,
                 organization, personal_phone, home_phone)


def _find_contact_by_parameters() -> dict:
    """
    Функция _find_contact_by_parameters
    собирает параметры по которым нужно произвести поиск
    и передает их в функцию поиска,
    возвращает найденный контакт
    :return: dict
    """
    print("Введите характеристики по которым "
          "хотите найти контакт в телефонной книге, "
    ...
    contact = find_contact(name, last_name, surname,
                           organization, personal_phone, home_phone)

    return contact


def _find_contact_by_parameters_by_parameters() -> None:
    contact = _find_contact_by_parameters()
